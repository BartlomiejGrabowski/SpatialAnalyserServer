...
class Ui_SendSHP(Client):
    def setupUi(self, SendSHP):
        ...
        QtCore.QObject.connect(self.shp_clear, QtCore.SIGNAL("clicked()"), self.clearFields)
        ...
        QtCore.QObject.connect(self.db_table, QtCore.SIGNAL("editingFinished()"), self.dbEntryReleased)
    ...
    def gui_client_send_shp_to_postgres(self):
        self.fName = str(self.shp_file.text())
        self.tabName = str(self.db_table.text())
        if self.fName == '':
            self.error_label.setText("<font color='red'>First select the shp file.</font>")
        if self.tabName == '':
            self.table_error_label.setText("<font color='red'>First fill out table name entry.</font>")
        else: 
